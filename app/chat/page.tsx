...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (currentUser) {
      loadUsers()
      // Real-time heartbeat every 5 seconds for online status
      ...
      const onlineStatusUpdate = setInterval(() => {
        updateOnlineStatus()
      }, 5000)

      // Reduced user list refresh since we have real-time updates
      const userListRefresh = setInterval(() => {
        if (!searchQuery.trim()) { // Only auto-refresh when not searching
          loadUsers()
        }
      ...
      return () => {
        ...
        clearInterval(onlineStatusUpdate)
        clearInterval(userListRefresh)
      }
    }
  ...
  useEffect(() => {
    if (selectedUser && currentUser) {
      loadMessages()
      // Real-time message checking every 5 seconds
  ...
  const updateOnlineStatus = async () => {
    try {
      const data = await apiCall(apiEndpoints.onlineStatus, {
        method: 'GET',
        credentials: 'include'
      })
      ...
      if (data.success) {
        ...
        setUsers(prevUsers => 
          prevUsers.map(user => ({
            ...user,
            is_online: data.users.some((onlineUser: any) => onlineUser.id === user.id)
          }))
        )
      }
    } catch (error) {
  ...
  const checkForNewMessages = async () => {
    ...
    try {
      const token = localStorage.getItem("blog_token")
      const lastMessageTime = messages.length > 0 ? messages[messages.length - 1]?.created_at : ''

      const data = await apiCall(`${apiEndpoints.newMessages}&chat_user_id=${selectedUser.id}&since=${lastMessageTime}`, {
        method: 'GET',
        headers: { Authorization: `Bearer ${token}` },
        credentials: 'include'
      })
